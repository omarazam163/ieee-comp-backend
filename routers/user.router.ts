...
body("email").exists({checkFalsy:true}).isEmail().custom(async (email:string)=>await uniqueEmail(email)),
(body("password").exists({checkFalsy:true}).isLength({min:6})),
body("userName").exists({checkFalsy:true}).isLength({min:3}).custom(async (userName:string)=>await uniqueUserName(userName)),
